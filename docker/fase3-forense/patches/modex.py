...
from volatility3.plugins.windows import pslist, dlllist, simple_pteenum, pedump
    ...
    def get_requirements(cls) -> List[interfaces.configuration.RequirementInterface]:
        return [
            ...
            requirements.PluginRequirement(name='simple_pteenum',
                                           ...
                                           version=(0, 9, 0)),
            requirements.PluginRequirement(name='pedump',
                                           plugin=pedump.PEDump,
                                           version=(1, 0, 0)),
                    ...
                    if module_name.casefold() == module_supplied:
                        ...
                        if module_base_address is not None and module_size is not None:
                            if is_detect_option_supplied:  # In this case, there is no need to dump the modules
                                ...
                                dumped_module_filename = f"pid.{process_id}.{module_name}.{entry.vol.offset:#x}.{entry.DllBase:#x}.dmp"
                                file_handle = pedump.PEDump.dump_pe(
                                                context=self.context,
                                                pe_table_name=pe_table_name,
                                                layer_name=process_layer_name,
                                                open_method=self.open,
                                                file_name=dumped_module_filename,
                                                base=entry.DllBase
                                            )
                                if file_handle:
                                    # file_handle.close()
